...
Current direct target-company support is Anthropic through Greenhouse, NVIDIA through Workday, Google through official Google Careers pages, Meta through Meta's official Careers GraphQL operation, and Tesla through Tesla's official careers API. Meta may report `failed` if the GraphQL endpoint returns 400 from the runner; Tesla may report `failed` if the endpoint returns 403. Apple remains a best-effort placeholder and reports `not_implemented` until a direct adapter exists. Simplify New Grad and SpeedyApply 2027 remain backup curated sources.
