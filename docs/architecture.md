...
Fetchers load jobs from public sources. `GreenhouseFetcher` supports Anthropic through the public board API. `GitHubMarkdownAdapter` reads raw Markdown from curated public repos. Placeholder company adapters return empty lists with clear warnings and source health status `not_implemented` until stable public parsers are implemented.

Every source becomes a normalized `Job` model. This gives filtering, scoring, email, and state code one common shape.

Filtering is deterministic and cheap. It classifies role family, checks role keywords, seniority, location, internships, citizenship, clearance, and sponsorship language before Gemini is considered. Ambiguous families such as fellowship/program, security, finance, legal, sales, product, and support cannot rule-only alert.
