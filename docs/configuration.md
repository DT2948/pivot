...
Current direct target-company support is Anthropic through Greenhouse, NVIDIA through Workday, Google through official Google Careers pages, and Tesla through Tesla's official careers API. Tesla may report `failed` if the endpoint returns 403 from the runner. Meta and Apple remain best-effort placeholders and report `not_implemented` until direct adapters exist. Simplify New Grad and SpeedyApply 2027 remain backup curated sources.
